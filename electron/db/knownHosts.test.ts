...
import { hostKeyFingerprint, trustHostKey, verifyHostKey } from './knownHosts'
...
describe('verifyHostKey', () => {
  it('rejects an unknown host until it is explicitly trusted and pinned', () => {
    const file = tmpStore()
    const outcome = verifyHostKey(file, 'bastion:22', keyA)
    expect(outcome).toEqual({ trusted: false, firstUse: true, presented: hostKeyFingerprint(keyA) })
    expect(fs.existsSync(file)).toBe(false)
    trustHostKey(file, 'bastion:22', keyA)
  ...
  it('rejects a known host whose key changed, reporting both fingerprints', () => {
    ...
    expect(outcome.firstUse).toBe(false)
    if (outcome.firstUse) return
  ...
  it('pins per host:port, so a different port is its own first use', () => {
    const file = tmpStore()
    trustHostKey(file, 'bastion:22', keyA)
    expect(verifyHostKey(file, 'bastion:2222', keyB)).toEqual({
      trusted: false,
      firstUse: true,
      presented: hostKeyFingerprint(keyB),
    })
  })
})
