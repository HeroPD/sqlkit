...
export type ColumnAlter = {
  ...
  default?: string | null
  comment?: string | null
  /** Create-table only: emit an auto-increment identity + inline PRIMARY KEY. */
...
// The engine's auto-increment identity clause, placed before an inline PRIMARY
// KEY. SQLite is empty: an INTEGER PRIMARY KEY column is already a rowid alias.
export const autoIncrementClause = (engine: Engine): string =>
  engine === 'postgresql'
...
// A short label for the auto-increment mechanism, shown read-only in the
// column's Default cell so the generated behavior is visible while editing.
...
const createColumnDefinition = (column: ColumnAdd, engine: Engine, inlinePrimaryKey = false): string => {
  const dialect = dialectFor(engine)
  ...
  // Auto-increment carries the engine's identity clause; the primary key is a
  // separate constraint, except SQLite where it must be inline (rowid alias). A
  // DEFAULT would fight the generator, so it is skipped.
...
export function buildCreateTable(
  ...
  const dialect = dialectFor(engine)
  // SQLite auto-increment must declare its PRIMARY KEY inline (rowid alias); fold
  // a matching single-column PK constraint into the column so it is not declared
  // twice. Other engines keep the PK as its own table-level constraint.
  const inlinePkColumns = new Set<string>()
  let tableConstraints = constraints
  if (engine === 'sqlite') {
    for (const column of columns) if (column.autoIncrement) inlinePkColumns.add(column.name.trim())
    tableConstraints = constraints.filter(
      (constraint) => !(constraint.type === 'PRIMARY KEY' && constraint.columns?.length === 1 && inlinePkColumns.has(constraint.columns[0]!)),
    )
  }
  const pkCount = tableConstraints.filter((constraint) => constraint.type === 'PRIMARY KEY').length + inlinePkColumns.size
  if (pkCount > 1) throw new Error(t('write.singlePrimaryKey'))
  const definitions = [
    ...columns.map((column) => createColumnDefinition(column, engine, inlinePkColumns.has(column.name.trim()))),
    ...tableConstraints.map((constraint) => buildCreateConstraintDefinition(constraint, engine)),
