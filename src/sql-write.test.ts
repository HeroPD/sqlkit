...
describe('buildCreateTable', () => {
  ...
  it('emits auto-increment as an identity column plus a primary key constraint', () => {
    const auto = [{ name: 'id', dataType: 'bigint', nullable: false, default: null, comment: null, autoIncrement: true }]
    const pk = [{ name: 'pk', type: 'PRIMARY KEY' as const, columns: ['id'] }]
    const pg = buildCreateTable(users, auto, pk, [], 'postgresql')[0]
    expect(pg).toContain('"id" bigint GENERATED BY DEFAULT AS IDENTITY')
    expect(pg).toContain('CONSTRAINT "pk" PRIMARY KEY ("id")')
    const my = buildCreateTable({ schema: null, name: 'accounts', kind: 'table' }, auto, pk, [], 'mysql')[0]
    expect(my).toContain('`id` bigint AUTO_INCREMENT')
    expect(my).toContain('CONSTRAINT `pk` PRIMARY KEY (`id`)')
    const ms = buildCreateTable({ schema: 'dbo', name: 'accounts', kind: 'table' }, auto, pk, [], 'sqlserver')[0]
    expect(ms).toContain('[id] bigint IDENTITY(1,1)')
    expect(ms).toContain('CONSTRAINT [pk] PRIMARY KEY ([id])')
  })

  it('folds a SQLite auto-increment column and its PK constraint into an inline INTEGER PRIMARY KEY', () => {
    const auto = [{ name: 'id', dataType: 'integer', nullable: false, default: null, comment: null, autoIncrement: true }]
    const out = buildCreateTable(
      { schema: null, name: 'accounts', kind: 'table' }, auto, [{ name: 'pk', type: 'PRIMARY KEY', columns: ['id'] }], [], 'sqlite',
    )[0]
    expect(out).toContain('"id" integer PRIMARY KEY')
    expect(out).not.toContain('CONSTRAINT') // folded inline, not a separate table constraint
  })

  it('rejects a SQLite auto-increment column plus a primary key on another column', () => {
    const cols = [
      { name: 'id', dataType: 'integer', nullable: false, default: null, comment: null, autoIncrement: true },
      { name: 'team_id', dataType: 'integer', nullable: false, default: null, comment: null },
    ]
    expect(() => buildCreateTable(
      { schema: null, name: 't', kind: 'table' }, cols, [{ name: 'pk', type: 'PRIMARY KEY', columns: ['team_id'] }], [], 'sqlite',
    )).toThrow(/only one primary key/i)
  })
})
